...
from deepkin.models.char_vocab import BOS_ID, EOS_ID, text_to_id_sequence
...
def gather_gpt_single_sequence(text_lines: List[str], doc_ends, max_seq_len) -> List[int]:
    ...
    while len(id_list) > 0:
        if (len(seq_char_ids) + len(id_list) + 2) < max_seq_len:
            seq_char_ids.extend(id_list)
        else:
            return [BOS_ID] + seq_char_ids + [EOS_ID]
        start_line = (start_line + 1) % num_text_lines
        id_list = text_to_id_sequence(text_lines[start_line % num_text_lines])
    return [BOS_ID] + seq_char_ids + [EOS_ID]
...
def char_gpt_data_collate_wrapper(batch_items):
    device = batch_items[0][0]
    batch_chars_list = []
    batch_input_sequence_lengths = []

    for bidx,data_item in enumerate(batch_items):
        (rank,char_ids_list) = data_item
        batch_chars_list.extend(char_ids_list)
        batch_input_sequence_lengths.append(len(char_ids_list))
    input_sequence_lengths = batch_input_sequence_lengths
    chars = torch.tensor(batch_chars_list).to(device)

    input_with_eos = True
    seq_len = max(input_sequence_lengths)
    # Using length: length-1 sot that last <EOS> token doesn't get processed
    input_masks = [torch.zeros(length, dtype=torch.bool, device=chars.device) for length in input_sequence_lengths]
    if input_with_eos:
        for i in range(len(input_masks)):
            input_masks[i][-1] = True
    input_masks_padded = pad_sequence(input_masks, batch_first=True, padding_value=1)  # Shape: N x S

    decoder_mask = generate_square_subsequent_mask(seq_len, chars.device)

    data_item = (chars, input_sequence_lengths, input_masks_padded, decoder_mask)
    return data_item
