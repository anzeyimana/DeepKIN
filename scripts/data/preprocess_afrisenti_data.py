...
def process_afrisenti_sentences(ffi, lib, in_file_path, out_file_path):
    ...
    with progressbar.ProgressBar(max_value=(len(Lines)), redirect_stdout=True) as bar:
        for iter in range(1,len(Lines)):
            if (iter % 100) == 0:
                bar.update(iter)
            parsed_file.write(parse_text_to_morpho_sentence(ffi, lib, Lines[iter].split('\t')[1]).to_parsed_format() + "\n")
    parsed_file.close()
