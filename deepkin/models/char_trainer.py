...
from deepkin.models.char_data import CharGPTDataset, char_gpt_data_collate_wrapper
from deepkin.models.char_gpt_model import CharGPT
...
def validation_loop(model: DDP, device,
    ...
    for batch_idx, batch_data_item in enumerate(validation_data_loader):
        (char_ids, char_id_lengths, input_masks_padded, decoder_mask) = batch_data_item
        loss, nll_loss = model(char_ids, char_id_lengths, input_masks_padded, decoder_mask)
    ...
    if dist.get_rank() == 0:
        print(time_now(),
              ...
              'VALIDATION LOSS:',
              'CHAR:', "{:.6f}".format(loss_aggr.item() / loss_Z),
              'NLL_LOSS:',
              'CHAR:', "{:.6f}".format(nll_loss_aggr.item() / loss_Z),
...
def eval_loop(model: DDP, device, validation_data_loader):
    world_size = dist.get_world_size()
    model.eval()
    model.zero_grad(set_to_none=True)
    ...
    if dist.get_rank() == 0:
        print(time_now(),
              ...
              'CHAR:', "{:.6f}".format(loss_aggr.item() / loss_Z),
              'NLL_LOSS:',
              'CHAR:', "{:.6f}".format(nll_loss_aggr.item() / loss_Z), flush=True)
        sys.stdout.flush()
        ...
        if int(total_steps % (accumulation_steps // world_size)) == 0:
            ...
            if (dist.get_rank() == 0):
                print(time_now(),
                      ...
                      'Warmup Iters: ', "{}".format(lr_scheduler.warmup_iter),
                      'OBJ: CHAR:', "{:.6f}".format(loss_aggr.item() / count_items),
                      'NLL_OBJ: CHAR:', "{:.6f}".format(nll_loss_aggr.item() / count_items),
...
def train_fn(rank, args):
    ...
    end_iters =  args.char_num_iters
    warmup_iter =  args.char_warmup_iters
    peak_lr = args.char_peak_lr #
    wd = args.wd # 0.01
    ...
    model = CharGPT(max_seq_len=args.char_max_seq_len).to(device)
    model.float()
    model = DDP(model, device_ids=[rank])
    model.float()

    curr_save_file_path = home_path + (f"models/char_gpt_model_{date_now()}.pt")

    best_valid_loss = 999999.9

    if (dist.get_rank() == 0):
        print('---------------------------------- CharGPT Model Size ----------------------------------------')
    ...
    num_train_loops = math.ceil(end_iters * args.char_accumulation_steps / args.char_number_of_load_batches)
    ...
    if (dist.get_rank() == 0):
        ...
        print('number_of_load_batches: ', args.char_number_of_load_batches)
        print('accumulation_steps: ', args.char_accumulation_steps)
        print('batch_size: ', args.char_batch_size, 'sequences')
        print('effective_batch_size: ', args.char_batch_size * args.char_accumulation_steps, 'sequences')
    ...
    np.random.seed(123)
    random.seed(123)
    dev_gpt_dataset = CharGPTDataset(dev_text, dev_text_doc_ends,
                                    2000,
                                    max_seq_len=args.char_max_seq_len)
    validation_data_loader = DataLoader(dev_gpt_dataset, batch_size=1,
                                      collate_fn=char_gpt_data_collate_wrapper,
    ...
    total_steps = int(lr_scheduler.num_iters * args.char_accumulation_steps)

    if (dist.get_rank() == 0):
        print(time_now(), 'Start training for', num_train_loops, 'loops ({} iterations)'.format(args.char_num_iters),
              flush=True)
    ...
    with progressbar.ProgressBar(initial_value=curr_loops, max_value=num_train_loops, redirect_stdout=True) as bar:
        ...
        while lr_scheduler.num_iters < lr_scheduler.end_iter:
            ...
            gpt_dataset = CharGPTDataset(text_corpus_lines, text_corpus_doc_ends,
                                     args.char_number_of_load_batches * args.char_batch_size // dist.get_world_size(),
                                     max_seq_len=args.char_max_seq_len)

            training_data_loader = DataLoader(gpt_dataset, batch_size=args.char_batch_size, collate_fn=char_gpt_data_collate_wrapper,
            ...
            total_steps, best_valid_loss = train_loop(model, device, scaler, optimizer,
                                                      ...
                                                      curr_save_file_path, args.char_accumulation_steps,
