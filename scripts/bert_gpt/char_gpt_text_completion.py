...
from deepkin.models.char_vocab import BOS_ID, text_to_id_sequence, EOS_ID, id_sequence_to_text
from deepkin.utils.misc_functions import normalize_kinya_text


def beam_search_lm_inference(seed_sentence, char_gpt: CharGPT, beam_size=20, max_length=512):
    ...
    with progressbar.ProgressBar(max_value=max_length, redirect_stdout=True) as bar:
        for itr in range(max_length):
            ...
            _, next_syllabe_probs = char_gpt.predict(syllabe_ids, syllabe_id_lengths)  # (N, |C|)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max-output-length", type=int, default=200)
    ...
    char_gpt = CharGPT(max_seq_len=args.char_max_seq_len).to(device)
    char_gpt.float()
    KINLP_HOME = (os.environ['KINLP_HOME']) if ('KINLP_HOME' in os.environ) else ('/opt/KINLP')
    home_path = KINLP_HOME + "/"
    state_dict = torch.load(home_path + "models/char_gpt_final_best_valid_2023-01-31_base_2023-01-25.pt", map_location=map_location)
    char_gpt.load_state_dict(state_dict['model_state_dict'])
    del state_dict
    char_gpt.eval()
    ...
    start = time.time()
    pending_texts, complete_texts = beam_search_lm_inference(seed_sentence, char_gpt, beam_size=beam_size, max_length=max_length)
    end = time.time()

    while True:
        print('\n\nInput: {}\n'.format(seed_sentence))

        print('\nCharGPT: PENDING:>> after {:.2f} seconds:\n'.format(end - start))
        ...
        if seed_sentence in {'exit', 'EXIT', 'e', 'E', 'quit', 'QUIT', 'q', 'Q'}:
            ...
            sys.exit(0)
        start = time.time()
        pending_texts, complete_texts = beam_search_lm_inference(seed_sentence, char_gpt, beam_size=beam_size,
