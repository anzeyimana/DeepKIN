...
def handle_appostrophe(w:str,l:str):
    w = w.strip().rstrip()
    l = l.strip().rstrip()
    ...
    appo = ['‘', '’', '\'', '`']
    for aaappp in appo:
        if w.startswith(aaappp):
            w = w[1:]
    for aaappp in appo:
        if w.endswith(aaappp):
            w = w[:-1]
    if (len(w) == 0) or (len(l)==0):
        return [], []
    for aaappp in appo:
        if (aaappp in w) and (len(w) > 1):
            wp = [(ww + 'a') if (i == 0) else ww for i, ww in enumerate(w.split(aaappp))]
            lp = [l.replace('B-', 'I-') if ((l[0] == 'B-') and (i > 0)) else l for i in range(len(wp))]
            words.extend(wp)
            labels.extend(lp)
            return words, labels
    return [w],[l]
def parse_ner_dataset(ffi, lib, in_file_path, out_file_path):
